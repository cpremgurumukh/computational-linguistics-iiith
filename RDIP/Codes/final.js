...
function dropdownchange() {
  ...
  if (x == 'english') {
    ...
    res.innerHTML = '';
    seldesc.innerHTML =
      '<br><br><b><center>Form a sentence (Declarative or Interrogative or any other type) from the given words</b></center>';
...
function quizzes() {
  ...
  desc2.innerHTML =
    '<br><ol><li>???? ???? ??? ?? ???</li><li>??? ???? ???? ?? ???</li><li>??? ???? ???? ?? ???</li><li>??? ???? ?? ???</li><li>??? ?? ????? ?? ?? ???? ????</li><li>??? ?? ?? ????? ?? ???? ????</li></ol>';
  desc3.innerHTML = '';
  select.innerHTML = '';
  seldesc.innerHTML = '';
  desc2.style.textAlign = 'left';
